  ...
  useEffect(() => {
    if (courseId) {
      const fetchTitle = async () => {
        ...
        setError(null);
        try {
          const res = await fetch(`/api/db?courseId=${courseId}`);
          const data = await res.json();
          setTitle(data?.data?.courseDetails?.records || []);
        } catch (err) {
                    ...
                    <SidebarMenuItem
                      className={
                        item.chapterId === chapterId
                          ? "bg-blue-100 shadow-sm rounded-lg p-2 text-black"
                          : `hover:bg-blue-100 hover:shadow-sm hover:rounded-lg p-2`
                      }
                    >
                      {item.title}
                    </SidebarMenuItem>
